...
async function placeOrder(userInfo) {
  ...
  let validationUser = validateOrderUser(userInfo);
  if (!validationUser.isValid) {
    return { success: false, errors: validationUser.errors };
  }
  const data = await createOrder(userInfo);
  if(data.status){
    return { success: true, data };
  }else{
    return { success: false, errors: [data.message || '建立訂單失敗'] };
  }
}
...
async function updatePaymentStatus(orderId, isPaid) {
  ...
  if(data.status){
    ...
    return { success: false, error: data.message || '更新訂單狀態失敗' };
  }
}
...
async function removeOrder(orderId) {
  ...
  if(data.status){
    ...
    return { success: false, error: data.message || '刪除訂單失敗' };
  }
}
...
function displayOrders(orders) {
  ...
  orders.forEach((order, index) => {
    ...
    console.log('----------------------------------------');
    console.log(`訂單編號：${orderData.id}`);
    console.log(`顧客姓名：${orderData.user.name}`);
    console.log(`聯絡電話：${orderData.user.phone}`);
    console.log(`寄送地址：${orderData.user.address}`);
    console.log(`付款方式：${orderData.user.paymentMethod}`);
    console.log(`訂單金額：${orderData.totalFormatted}`);
    console.log(`付款狀態：${orderData.paidText}`);
    console.log(`建立時間：${orderData.createdAt} (${orderData.daysAgo} 天前)`);
    console.log('----------------------------------------');
    console.log('商品明細：');
