...
async function fetchCart() {
  // 請實作此函式
  // 回傳 { carts: [...], total: 數字, finalTotal: 數字 }
...
async function addToCart(productId, quantity) {
  ...
  const response = await axios.post(`${BASE_URL}/api/livejs/v1/customer/${API_PATH}/carts`, dataObj);
...
async function updateCartItem(cartId, quantity) {
  ...
  const response = await axios.patch(`${BASE_URL}/api/livejs/v1/customer/${API_PATH}/carts`, dataObj);
  return response.data;
}
...
async function createOrder(userInfo) {
  ...
  const response = await axios.post(`${BASE_URL}/api/livejs/v1/customer/${API_PATH}/orders`, dataObj);
  return response.data;
}
...
async function updateOrderStatus(orderId, isPaid) {
  ...
  const response = await axios.put(`${BASE_URL}/api/livejs/v1/admin/${API_PATH}/orders`, dataObj, {
    headers: {
      authorization: ADMIN_TOKEN
    }
  });
